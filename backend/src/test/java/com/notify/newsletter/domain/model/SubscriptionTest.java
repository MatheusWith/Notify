...
class SubscriptionTest {
    ...
    void givenValidData_whenBuilt_thenSucceeds() {
        ...
        Subscription subscription = Subscription.builder().id(UUID.randomUUID()).email(email)
                .newsletterId(UUID.randomUUID()).token(UUID.randomUUID()).expiresAt(LocalDateTime.now().plusHours(24))
                .build();
    ...
    void givenNoSubscriberId_whenBuilt_thenSubscriberIdIsNull() {
        Subscription subscription = Subscription.builder().id(UUID.randomUUID())
                .email(new SubscriberEmail("user@example.com")).newsletterId(UUID.randomUUID()).token(UUID.randomUUID())
    ...
    void givenSubscriberId_whenBuilt_thenReturnsSubscriberId() {
        ...
        Subscription subscription = Subscription.builder().id(UUID.randomUUID())
                .email(new SubscriberEmail("user@example.com")).newsletterId(UUID.randomUUID())
